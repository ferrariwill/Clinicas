...
func SetupRoutes(r *gin.Engine, db *gorm.DB) {
	...
	usuarioService := services.NovoUsuarioService(usurioRepo, usuarioClinicaRepo, tipoUsuarioRepo, mailer)
	...
	agendaService := services.NovaAgendaService(agendaRepo, configuracaoRepo, pacienteRepo)
	prontuarioService := services.NovoProntuarioService(prontuarioRepo, pacienteRepo)
	planoTratamentoService := services.NovoPlanoTratamentoService(prontuarioService, pacienteService)
	planoTratamentoController := controllers.NovoPlanoTratamentoController(planoTratamentoService)
	atestadoRepo := repositories.NovoAtestadoRepository(db)
	atestadoService := services.NovoAtestadoService(atestadoRepo, pacienteRepo)
	medicoDashboardClinicoService := services.NovoMedicoDashboardClinicoService(prontuarioRepo, atestadoRepo)
	...
	fechamentoPeriodoRepo := repositories.NovoFechamentoPeriodoRepository(db)
	fechamentoFinanceiroService := services.NovoFechamentoFinanceiroService(db, lancamentoFinanceiroRepo, fechamentoPeriodoRepo, cobrancaService)
	fechamentoFinanceiroController := controllers.NovoFechamentoFinanceiroController(fechamentoFinanceiroService)
	...
	atestadoController := controllers.NovoAtestadoController(atestadoService, usurioRepo, pacienteRepo, auditLogRepo)
	medicoDashboardClinicoController := controllers.NovoMedicoDashboardClinicoController(medicoDashboardClinicoService)
	...
	clinicas := r.Group("/clinicas", middleware.Autenticado(), verificarPermissaoTipoUsuario.VerificaPermissaoTipoUsuario)
	{
		...
		clinicas.GET("/financeiro/fechamento/preview", fechamentoFinanceiroController.Preview)
		clinicas.GET("/financeiro/fechamento", fechamentoFinanceiroController.Listar)
		clinicas.GET("/financeiro/fechamento/:id", fechamentoFinanceiroController.Buscar)
		clinicas.POST("/financeiro/fechamento", fechamentoFinanceiroController.Criar)
		...
		clinicas.POST("/agenda/lote/preview", agendaController.PreviewLote)
		clinicas.POST("/agenda/lote", agendaController.CriarLote)
		...
		clinicas.GET("/cobrancas/relatorio-repasse-profissionais", cobrancaController.RelatorioRepasseProfissionais)
		clinicas.POST("/cobrancas", cobrancaController.Criar)
		clinicas.GET("/cobrancas/:id", cobrancaController.Detalhe)
		clinicas.GET("/atestados", atestadoController.Listar)
		clinicas.POST("/atestados", atestadoController.Criar)
		clinicas.GET("/prontuarios", prontuarioController.Listar)
		clinicas.POST("/prontuarios", prontuarioController.Criar)
		clinicas.POST("/plano-tratamento", planoTratamentoController.Registrar)
		clinicas.GET("/me/dashboard-clinico", medicoDashboardClinicoController.DashboardClinico)
