...
func (r *pacienteRepository) BuscarPorIDClinica(id, clinicaID uint) (*models.Paciente, error) {
	var paciente models.Paciente
	err := r.db.Where("id = ? AND clinica_id = ?", id, clinicaID).First(&paciente).Error
	if err != nil {
		return nil, err
	}
	return &paciente, nil
