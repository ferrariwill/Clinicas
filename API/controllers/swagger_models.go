...
// MetricasOperacionaisSwagger faturamento e taxa de no-show no período.
type MetricasOperacionaisSwagger struct {
	PeriodoInicio            string  `json:"periodo_inicio" example:"2026-04-01"`
	PeriodoFim               string  `json:"periodo_fim" example:"2026-05-01"`
	Faturamento              float64 `json:"faturamento" example:"12500.50"`
	AgendamentosConsiderados int     `json:"agendamentos_considerados" example:"120"`
	TotalFaltas              int     `json:"total_faltas" example:"8"`
	TaxaNoShowPercentual     float64 `json:"taxa_no_show_percentual" example:"6.67"`
}

// CriarProntuarioRequest corpo para novo registro de prontuário.
type CriarProntuarioRequest struct {
	PacienteID uint   `json:"paciente_id" binding:"required" example:"1"`
	Titulo     string `json:"titulo" binding:"required" example:"Evolução"`
	Conteudo   string `json:"conteudo" binding:"required" example:"Paciente evolui bem."`
}

// AtualizarProntuarioRequest edição permitida até 24h após criação.
type AtualizarProntuarioRequest struct {
	Titulo   string `json:"titulo" binding:"required"`
	Conteudo string `json:"conteudo" binding:"required"`
}

// ProntuarioRegistroSwagger registro clínico persistido.
type ProntuarioRegistroSwagger struct {
	ID             uint   `json:"id" example:"1"`
	PacienteID     uint   `json:"paciente_id" example:"1"`
	ClinicaID      uint   `json:"clinica_id" example:"1"`
	ProfissionalID uint   `json:"profissional_id" example:"2"`
	Titulo         string `json:"titulo" example:"Evolução"`
	Conteudo       string `json:"conteudo" example:"..."`
	CreatedAt      string `json:"created_at" example:"2026-04-05T10:00:00Z"`
}